#!/usr/bin/python
# This is a Python 2 file, which I can't use (may convert at a later date...)
import subprocess
import os
import sys
import re
...
def runAndCheckSizes():
    ...
    lines = set([x.replace(" ", "")
                for x in output.strip().lower().split('\n')])
    types = [
        ("int", 4),
        ("short", 2),
        ("long", 8),
    ...
    for typ in types:
        ...
        if (expected_output_format % typ).replace(" ", "") not in lines:
            exitWithError(
                "ERROR: couldn't find type %s (or it has the incorrect value) in sizes output" % typ[0])
...
def build(make_arg, filename):
    ...
    if not os.path.isfile(filename):
        exitWithError(
            "ERROR: %s binary missing, did you rename it?" % filename)
    print "Ok!"
